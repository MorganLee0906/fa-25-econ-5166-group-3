...
-   Load in the shp (geometric data) of Manhattan
-   Load in the air quality data of 2024 and 2025
-   Combine the two through geometric Processing
-   Merge 2024 and 2025 Manhattan air quality data into a processed data `climate`
------------------------------------------------------------------------
...
-   **Source Data Sets**:
    ...
    -   Description: The taxi zones in geometric form separated in grids (See in the below map)

    2.  `ad_viz_plotval_data.csv`

    -   Description: Air quality data from 2025, New York; preprocessed on the source website

    3. `ad_viz_plotval_data_2024.csv`

    -   Description: Air quality data from 2024, New York; preprocessed on the source website

------------------------------------------------------------------------

## What does this file output?

-   `processed_pm2.5_AQI.csv`
    -   Description: Clean data combining the daily (sometimes 3-daily) air quality data of four stations.
...
evile_df_2025 = sites_in_mht %>%
  ...
  rename(
    ...
    "AQI" = Daily.AQI.Value,
    "region" = zone) %>%
  select(Date, region, PM2.5, AQI)
```

```{r}
evile_df_2024 = read.csv("C:/114-1_Autumn/Data_science_and_social_inquiry/data/air_quality/ad_viz_plotval_data_2024.csv") %>%
  filter(!is.na(Site.Latitude) & !is.na(Site.Longitude)) %>%
  st_as_sf(coords = c("Site.Longitude", "Site.Latitude"), crs = 4326) %>%
  st_intersection(mht_shp) %>%
  as.data.frame() %>%
  rename(
    "PM2.5"= Daily.Mean.PM2.5.Concentration,
    "AQI" = Daily.AQI.Value,
