...
##        The main algorithm is to Random Forest to synthesize the counterfactual (control) group of volume data, then the treatment effect can be identified.
...
## The results is saved in /findings/hvfhv/volume
## ---- 套件 ----
...
## ---- 3. 讀取 2024-01 ~ 2024-12 & 2025-01 ~ 2025-06 fhvhv 資料 ----
...
trips_raw <- lapply(data_files, read_parquet, as_data_frame = TRUE) |>
  bind_rows()

## ---- 4. 處理時間欄位、只保留平日 ----
trips <- trips_raw %>%
  dplyr::mutate(
    pickup_datetime = lubridate::ymd_hms(pickup_datetime, tz = "America/New_York")
  ) %>%
...
## ---- 5. 每小時 target y（treated_loc 車次，整段期間）----
hourly_y <- trips %>%
...
out_dir <- "Volume/random_forest_hourly"
...
p_pc <- ggplot(pc_var_df, aes(x = PC, y = CumVarExpl)) +
  ...
  theme(
    plot.title = element_text(        # ★ 這裡改標題大小
      size = 8
    )
  )
ggsave(
...
## ---- 11. 用 PCA 模型算「所有期間」的 PC scores（這裡用 dense，若記憶體足夠）----
cat("Computing PC scores for all periods with predict()...\n")
X_all_dense <- as.matrix(X_all_sparse)  # 你機器記憶體 OK 的話可以接受這步
...
pred_df <- rf_data_hour %>%
  dplyr::mutate(
    y_hat   = y_hat_all,
    # ⭐ 依照你的需求，把 period 分成：train、2024/01-03、2025/01-03
...
## ---- 17. 聚合成每天 05:00–21:00 的平均 y / ŷ（畫 2024/01–2025/06）----
daily_avg <- pred_df %>%
...
## ---- 18. 畫圖：2024/01–2025/06 每天 05:00–21:00 的 y vs ŷ ----
