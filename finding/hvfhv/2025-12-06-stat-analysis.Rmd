...
The policy took effect on 2025/01/05. The simplest way to evaluate changes in demand would be to plot the average daily ridership within the CBD (i.e., a regression discontinuity in time). However, total ridership/tips is highly volatile and confounded by many external factors, making it difficult to isolate the policy effect.
To address this, we propose using a random forest　(RF) model trained on pre-policy data to synthesize counterfactual hourly ridership volumes/tips after 2025/01/05, using ridership patterns outside the CBD as predictors. This allows us to estimate what HVFHV demand would have been in the absence of the policy. Because the ridership patterns outside the CBD are high-dimensional, we also use principal component analysis (PCA) to extract a set of informative and tractable features to use as predictors.　The algorithm of PCA and RF is in other .rmd files to make sure the work is obey to “交付規範”.
- **Main Findings and Takeaways**:
The policy does not appear to reduce HVFHV ridership volume within the CBD, suggesting that the demand for HVFHV trips into the CBD is relatively inelastic.　The policy-induced tax is indirectly shifted onto drivers, reflected in a statistically significant decline in tipping behavior.
- **Future Direction**:　No.
